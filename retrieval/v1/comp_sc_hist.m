...
function [sc_hist] = comp_sc_hist(dists,angles,n_dist,n_theta)
	...
	if 1
		...
		logbase		= 1.5;
		mean_dis	= mean(dists(:));
		a			= (logbase^(.75*n_dist)-1)/mean_dis;	% normalize coefficient
		dists		= floor(log(a*dists+1)/log(logbase));	% +1 to deal with 0 distance
		dists		= max(dists,1);
	...
	%- shape context, the following code is slow, rewrite if necessary
