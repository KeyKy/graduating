function checkangle2()
%%%Prewitt Operator Corner Detection.m  
    %%%时间优化--相邻像素用取差的方法求Harris角点  
     %%   
     clear;  
%      Image = imread('F:\sketch\测试图片\m39_wangfei_0.png');                 % 读取图像  
     [~,~,Image]  =imread('F:\sketch\total\m0_kangyang_0.png');  %输入手绘图
    ...
    Image = imageBoundaryExpandFixSize(rescaleBinaryImg, 200, 200);

%      Image = im2uint8(rgb2gray(Image)); 
%     [image, boundImg, rescaleImg, rescaleBinaryImg, fixExpandImg, Image]
%     = preprocessProjImage('F:\sketch\测试图片\m39_6_1.png',200);%投影图的预处
     ...
     for i=boundary:nrow-boundary+1   
        for j=boundary:ncol-boundary+1  
            ...
            if Image(i,j-1)>Image(i,j)-t && Image(i,j-1)<Image(i,j)+t    
             ...
             if nlike>=2 && nlike<=6  
                 Corner(i,j)=1;%如果周围有2~6个相似点，那(i,j)可能就是角点  
             end;  
         end;  
     end; 
%      imshow(Corner);
    ...
    disp('角点个数');  
     disp(count)  
     fig1 = figure(1),imshow(Image);      % display Intensity Image  
     hold on;    
     for i=boundary:nrow-boundary+1   
        for j=boundary:ncol-boundary+1  
             column_ave=0;  
     ...
     % 用算数平均数作为角点坐标，如果改用几何平均数求点的平均坐标，对角点的提取意义不大  
             ...
             if k>0 %周围不止一个角点       
                 new_x = column_ave/k; new_y = row_ave/k;
                 plot( new_x,new_y ,'g.'); hold on;
